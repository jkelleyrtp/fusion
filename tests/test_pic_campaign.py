...
class CampaignTests(unittest.TestCase):
    ...
    def test_six_coil_track_study_records_settled_electron_paths(self) -> None:
        ...
        for item in configurations:
            ...
            self.assertEqual((item.track, item.track_after, item.track_samples), (64, 5e-7, 65536))
            self.assertAlmostEqual(item.track_every * item.dt, 2e-12, delta=1e-24)
            self.assertEqual(item.dt * item.inject_every, 4e-12)
    ...
    def test_six_coil_multi_gun_study_splits_current_over_face_guns(self) -> None:
        argv = commands(Path("/campaign"), "a" * 40, "six-coil-multi-gun")
        configurations = {Path(command[3]).name: ion_parser().parse_args(command[2:]) for command in argv}
        self.assertEqual(len(configurations), 8)
        for item in configurations.values():
            validate_coupled(item)
            self.assertEqual((item.coils, item.coil_current, item.gas_pa, item.inject_per_step), (6, 30000, 1e-3, 12))
        self.assertEqual(sorted({item.guns for item in configurations.values()}), [2, 3, 6])
        self.assertEqual(sorted({item.current_a for item in configurations.values()}), [30, 100, 300, 1000])
        self.assertEqual(configurations["guns6_1000A_20keV"].ion_dt, 2e-10)
    ...
    def test_six_coil_multi_gun_check_varies_particles_cycles_and_pressure(self) -> None:
        ...
        self.assertEqual(configurations["guns6_1000A_20keV_ppc4"].ion_dt, 2e-10)
    ...
    def test_six_coil_multi_gun_scale_varies_mesh_coils_and_feed(self) -> None:
        argv = commands(Path("/campaign"), "a" * 40, "six-coil-multi-gun-scale")
        configurations = {Path(command[3]).name: ion_parser().parse_args(command[2:]) for command in argv}
        self.assertEqual(len(configurations), 8)
        for item in configurations.values():
            validate_coupled(item)
            self.assertEqual((item.guns, item.gas_pa, item.inject_per_step), (6, 1e-3, 12))
            self.assertAlmostEqual(item.dt * item.inject_every, 4e-12)
        strong = configurations["guns6_1000A_20keV_60kAt"]
        self.assertEqual((strong.coil_current, strong.dt, strong.ion_dt, strong.cycles), (60000, 1e-12, 2e-10, 16))
        self.assertEqual(configurations["guns6_3000A_20keV_n49"].nodes, 49)
        self.assertEqual(configurations["guns6_3000A_20keV"].ion_dt, 1e-10)
    ...
    def test_six_coil_deuteron_study_injects_atomic_ions(self) -> None:
        argv = commands(Path("/campaign"), "a" * 40, "six-coil-deuteron")
        configurations = [ion_parser().parse_args(command[2:]) for command in argv]
        self.assertEqual(len(configurations), 8)
        for item in configurations:
            validate_coupled(item)
            self.assertEqual((item.fuel, item.coils, item.current_a, item.gas_pa, item.dissociative_fraction),
                             ("D2", 6, 1, 1e-5, 0.05))
        species = [item.ion_gun_species for item in configurations if item.ion_gun_current > 0]
        self.assertEqual((species.count("atomic"), species.count("molecular")), (6, 1))
