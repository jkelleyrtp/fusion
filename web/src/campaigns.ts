...
const runLabel = (run: Run): string => `${format(run.energyEV)} eV · ${format(run.radiusM * 100)} cm${
...
export function renderCampaignHome(host: HTMLElement, catalog: Catalog, age: (study: Catalog["studies"][number]) => string): void {
  host.innerHTML = `<div class="page-title"><div><h1>Campaigns</h1><p>Completed experiments, newest first</p></div><span>${catalog.runs.length} runs</span></div>
    <div class="campaign-index">${catalog.studies.map(study => {
      ...
      return `<a class="campaign-row" href="${campaignHref(study.id)}"><div><h2>${escape(study.label)}</h2>
        <p>${runs.length} runs · ${range(runs.map(r => r.energyEV))} eV · ${range(runs.map(r => r.radiusM * 100))} cm coils</p></div>
        <span>${escape(age(study))} <b>→</b></span></a>`;
    }).join("")}</div>
    <h2 class="reference-heading">Reference analyses</h2>
    <a class="campaign-row" href="#space-charge"><div><h2>Space-charge pilot</h2><p>8 cases · stationary trajectory–Poisson feedback · partial results</p></div><span>View analysis →</span></a>`;
}
...
export function renderCampaignReport(heading: HTMLElement, details: HTMLElement, catalog: Catalog, run: Run, onPreview: (id: string) => void): void {
  ...
  details.innerHTML = `<h2>Individual runs</h2><p class="muted">Mean dwell is measured through each run’s observation window. † marks a censored lower bound.</p>
    ...
    <tbody>${members.map(r => `<tr><td><a href="${reportHref(r.id)}">${escape(runLabel(r))}</a></td>
      ...
      <td>${r.sweep ? `${r.sweep.gridR}×${r.sweep.gridZ} · gyro ${r.sweep.gyroFraction}` : "—"}</td></tr>`).join("")}</tbody></table></div>`;
}
